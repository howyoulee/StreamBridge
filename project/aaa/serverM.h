...
//struct user to store the information of each user (name + available time)
typedef struct user{
    string name;                     //name check
    int time_ava[10][2];
}User;

//store the basic information about backend server in main server
...
//connected client id with its input
typedef struct tcp_client{
    string id;
    vector<string> list;
}Client;

//struct TimeInterval used to store each time availability
typedef struct TimeInterval {
    int start;
    int end;
}TimeInterval;

//struct Query used to send the data request to backend server
...
void initSockUDP(){
    // Initialize server M IP address, port number
...
void requestEndServer(Client REQ, Server end_server){
    ...
    if(REQ.list.size() == 0){
        // cout << "^^^\n";
        no_exist.clear();
        // cout << "no_exist is emplty? " << no_exist.empty() << endl;
        return;
    }
    ...
    if (sent_bytes > 0) {
        ...
        if(end_server.id == "B"){
            ...
            for(int i = 0; i < RequestB.list.size(); i++){
                ...
                if(i != (RequestB.list.size() - 1))
                    cout << ", ";
            }
        }
        
        cout << " located at Server " << end_server.id << " Send to Server " << end_server.id << endl;
    }
...
void boot_up(){
    ...
    // for(int i = 0; i < ServerB.list.size(); i++)
    //     cout << ServerB.list[i] << endl;
...
vector<TimeInterval> getResult(Server end_server){
    ...
    // cout << "getResult(): waiting for response from Server " << end_server.id << "..." << endl;
...
vector<TimeInterval> getIntersection(vector<TimeInterval> timelist_1, vector<TimeInterval> timelist_2){
    vector<TimeInterval> result{};

    for(int i = 0; i < timelist_1.size(); i++){
        TimeInterval t1 = timelist_1[i];

        for(int j = 0; j < timelist_2.size(); j++){
            TimeInterval t2 = timelist_2[j];
...
void replyResult(vector<TimeInterval> time_list){

    memset(send_buf, 0, 1024);

    // cout << "replyResult(): send back result to client..." << endl;

    if(time_list.size() == 0){
        string a = "";
        strcpy(send_buf, a.c_str());
        send(tcp_sockfd_child, send_buf, sizeof(send_buf), 0);
        // cout << "There is no intersection in the final result." << endl;
        return;
    }

    vector<char> serialized_data = serialize(time_list);
    ssize_t send_bytes = send(tcp_sockfd_child, serialized_data.data(), serialized_data.size(), 0);
    if(send_bytes <= 0){
        perror("Message Send Error");
		close(tcp_sockfd_child);
		exit(1);
