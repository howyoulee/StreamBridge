...
#define ServerA_UDP_PORT 21427 // Server B port number
...
int udp_sockfd_M; // Server B datagram socket
int udp_sockfd_B;
struct sockaddr_in serverB_addr; // AWS address as a server & as a client
...
void createSockUDP(){
    ...
    if (udp_sockfd_B == FAIL) {
        perror("Error: server B socket open denied!\n");
        exit(1);
    }
}
...
void initSockUDP(){
    ...
    serverB_addr.sin_port = htons(ServerA_UDP_PORT); // Server B port number
}

void bindSockUDP(){
    if (::bind(udp_sockfd_B, (struct sockaddr *) &serverB_addr, sizeof(serverB_addr)) == FAIL) {
        perror("Error: Server B failed to bind UDP socket");
        exit(1);
    }

    cout << "Server B is up and running using UDP on port " << ServerB_UDP_PORT << endl;
}
...
void sendList(string list){
    ...
    ssize_t sent_bytes = sendto(udp_sockfd_B, list.c_str(), list.length(), 0, (struct sockaddr*)&serverM_addr, sizeof(serverM_addr));
    if (sent_bytes > 0) {
...
void boot_up(){
    ...
    initSockUDP();
    //1.3 bind socket with specified IP address and port number in server B
    bindSockUDP();
    
    //2. read file
    //vector<User> list_B for each user
    list_B = readFile(file);

    //check the whole list in server B
    for (const auto& user : list_B) {
    ...
    //     for(auto& row : list_B[i].time_ava){
    //         cout << "[ ";
    //         for(auto& element : row) {
    //             if(element != 0)
    //             cout << element << " ";
    ...
    name_list_B = getName(list_B);          //vector<string> type
    list_buf = getNameStr(name_list_B);     //vector -> string
    //cout << list_buf << endl;
...
vector<TimeInterval> schedule(){
    ...
    response = getRequestInfo(request, list_B);
