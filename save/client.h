...
void sendList(vector<string> list){
    ...
    if(send(tcp_sockfd_C, send_buf, strlen(send_buf), 0) == FAIL){
...
vector<string> recvNoExist(){
    ...
    if(recv(tcp_sockfd_C, recv_buf, strlen(recv_buf), 0) == FAIL){
    ...
    // if(nonlist.size() != 0)
    //     cout << "Client received the reply from Main Server using TCP over port: " << ServerM_TCP_PORT << ":< " << str << ">" << " do not exist." << endl;
